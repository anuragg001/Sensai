...
const EntryForm = ({ type, entries, onChange }) => {
    ...
    useEffect(() => {
        if (improvedContent && !isImproving) {
            ...
            toast.success("Descrption Improved successfully")
        }
        if (improveError) {
            toast.error(improveError.message || "Failed to load descrption")
        }
    }, [improvedContent, improveError, isImproving])

    const handleImproveDescrption = async () => {
        const descrption = watch("description")
        if (!descrption) {
            toast.error("Please enter descrption first")
            return;
        }
        await improveWithAIFn({

            current: descrption,
            type: type.toLowerCase() //n "experince","education","projects"
    ...
    return (
        <div className='space-y-4'>
            <div className='space-y-4'>{entries.map((item, index) => {
                return (
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <Button
                            ...
                            variant="ghost"
                            size="sm"
                            onClick={handleImproveDescrption}
